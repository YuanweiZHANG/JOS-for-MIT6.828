...
trap_init_percpu(void)
{
	...
	gdt[(GD_TSS0 >> 3) + i] = SEG16(STS_T32A, (uint32_t) (&thiscpu->cpu_ts),
					sizeof(struct Taskstate) - 1, 0); // Cautious: DO NOT forget this ts!
	gdt[(GD_TSS0 >> 3) + i].sd_s = 0;

	// Load the TSS selector (like other segment selectors, the
	// bottom three bits are special; we leave them 0)
	ltr(GD_TSS0 + (i << 3));  // Cautious: must have i << 3 here, because TSS is different of each CPU
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall != NULL) {
		...
		if (tf->tf_esp < UXSTACKTOP && tf->tf_esp >= (UXSTACKTOP - PGSIZE)) {
			...
			uesp = tf->tf_esp - sizeof(struct UTrapframe) - 4; // Cautious: leave an extra word to eip
		}
		else {
			uesp = UXSTACKTOP - sizeof(struct UTrapframe);
		}
		user_mem_assert(curenv, (void *)uesp, sizeof(struct UTrapframe), PTE_W);
		...
		if (uesp < UXSTACKTOP && uesp >= (UXSTACKTOP - PGSIZE)) {
			curenv->env_tf = *tf;
			curenv->env_tf.tf_esp = uesp;
